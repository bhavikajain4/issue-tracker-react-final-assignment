...
import "./Issue_Cards_Dashboard.css";
...
const Issue_Cards_Dashboard = ({
  ...
  useEffect(() => {
    if (filterbyassignee === "" && filterbypriority === "") {
      ...
      setFilter(
        [...list].filter((ele: any) => ele.assignee.id == filterbyassignee)
      );
    } else if (filterbyassignee === "" && filterbypriority !== "") {
      setFilter(
        [...list].filter((ele: any) => ele.priority == filterbypriority)
      );
    } else if (filterbyassignee !== "" && filterbypriority !== "") {
      setFilter(
        [...list].filter((ele: any) => ele.assignee.id == filterbyassignee)
      ...
      setFilter(
        [...filter].filter((ele: any) => ele.priority == filterbypriority)
      );
    }
  }, [filterbyassignee, filterbypriority]);
  return (
    <div className="status-issue-cards">
      <div className="issue-list">
        <p className="type-issue-cards">{t("TO DO")}</p>
        {todoList &&
      ...
      <div className="issue-list">
        <p className="type-issue-cards">{t("DEVELOPMENT")}</p>
        {inProgressList &&
      ...
      <div className="issue-list">
        <p className="type-issue-cards">{t("TESTING")}</p>
        {testing &&
          testing.map((issue) => {
            return <Card key={issue.id} issue={issue} />;
          })}
      ...
      <div className="issue-list">
        <p className="type-issue-cards">{t("COMPLETED")}</p>
        {completed &&
          completed.map((issue) => {
            return <Card key={issue.id} issue={issue} />;
          })}
...
export default Issue_Cards_Dashboard;
